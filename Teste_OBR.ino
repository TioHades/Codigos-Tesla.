...
void loop(){
  ...
  if(valor1 < fita && valor2 < fita)//Enguantos os sensores do meio não tiverem vendo a fita motor ON.
 {
 ...
 if(valor1 >= fita_verde_base && valor1 <= fita_verde_max && valor2 >= fita_verde_base && valor2 <= fita_verde_max) {
  //ISSO E PARA QUANDO TIVER UMA FITA VERDE VIRAR PARA OS DOIS LADOS.
   ...
   if(valor1 < fita && valor2 < fita)
 {
  ...
  if(valor3 <= fita_prata_max  && valor3 >= fita_prata_base){//Se o robor sentir a fita prata passar para o objetivo 2.
    Objetivo = 2;
  }
   while(Objetivo == 2){//Objetivo 2.
    //MODO RESGATE.
